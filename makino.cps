...
description = "Makino V33 3-axis V1.4.8";
vendor = "Makino";
...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      {id:COOLANT_AIR, on:7},
      {id:COOLANT_AIR_THROUGH_TOOL, on:77, off:9},// 3/9/26 ial
      {id:COOLANT_SUCTION},
...
function onSection() {
  ...
  if (isProbeOperation()) {
    ...
    // G170 activates the probe internally — no separate probe-start macro needed
...
function onSectionEnd() {
  ...
  if (isProbeOperation()) {
    // G170 deactivates the probe internally — no separate probe-end macro needed
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      var d = getSetting("outputToolDiameterOffset", true) ? diameterOffsetFormat.format(2) : ""; // 3-9-26 ial
      writeBlock(gPlaneModal.format(17));
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  var hOffset = getSetting("outputToolLengthOffset", true) ? hFormat.format(1) : ""; // 3/9/26 ial
...
function writeProbeCycle(cycle, x, y, z, P, F) {
  ...
  case "probing-xy-outer-corner":
    ...
    break;
  // ── Not supported by Makino EasySet ─────────────────────────────────────────
...
function getMakinoCorner(approach1, approach2) {
  var ax = approach(approach1);
  var ay = approach(approach2);
  if (ax > 0 && ay > 0) { return 3; } // Upper Right
  if (ax > 0 && ay < 0) { return 4; } // Lower Right
